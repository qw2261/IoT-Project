...
class ov2640(object):
    def __init__(self, sclpin=5, sdapin=4, cspin=15, resolution=OV2640_320x240_JPEG):
        self.sdapin=sdapin
        self.sclpin=sclpin
        self.cspin=cspin
        self.standby = False
...
class Buffer():
    def __init__(self):
        self.string = ""
        self.no = 0
    def addNew(self, new_str):
        if self.no >= 10:
            req.post(url, data=self.string)
            gc.collect()
            self.string = ""
            self.no = 0
        self.string += new_str
    ...
    try:
        ...
        for by in picbuf:
            if (c > howmany):
                break
            c += 1
            buffer.addNew(str(bytes([by[0]])) + 'zz')
            # f.write(bytes([by[0]]))
            # req.post(url, data=str(bytes([by[0]])))
        # f.close()
    except OSError:
